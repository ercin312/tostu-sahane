...
class WaiterPosNode {
  ...
  final double? price;
  /// Canlı katalog ürün id (varsa fiyat/id buradan gelir).
  final String? productId;
  final List<WaiterPosNode> children;

  /// Klasör: fiyat/ürün yok (alt seçim). Boş klasör de klasördür.
  bool get isFolder =>
      price == null && (productId == null || productId!.isEmpty);
  bool get isLeaf => !isFolder;

  WaiterPosNode copyWith({
    String? id,
    String? label,
    double? price,
    String? productId,
    List<WaiterPosNode>? children,
    bool clearPrice = false,
    bool clearProductId = false,
  }) {
    return WaiterPosNode(
      id: id ?? this.id,
  ...
  factory WaiterPosNode.fromJson(Map<String, dynamic> json) {
    ...
    return WaiterPosNode(
      ...
      children: rawChildren is List
          ? rawChildren
              .whereType<Map>()
              .map(
                (e) => WaiterPosNode.fromJson(Map<String, dynamic>.from(e)),
              )
              .toList()
          : const [],
    );
  ...
  Product toProduct(ProductCategory category) {
    final p = price ?? 0;
    return Product(
      id: productId?.isNotEmpty == true ? productId! : id,
      nameKey: label,
